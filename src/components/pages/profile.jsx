...
export const Profile = () => {
  ...
  const getPokemonData = async () => {
    try {
      ...
      const abilitiesUrl = res.data.abilities.map(({ ability }) => ability.url);
  ...
  return (
    <Main>
      ...
      <ImgContainer>
        <Img
          src={`https://img.pokemondb.net/artwork/avif/${id}.avif`}
          alt={`Default sprite of ${pokemonData.name}`}
        />
      </ImgContainer>
          ...
          {ability.map((element, index) => {
            return (
              <AccordionComp
                ...
                header={`${element.name} `}
                body={
                  element.effect_entries[1].language.name === "en"
                    ? element.effect_entries[1].effect
                    : element.effect_entries[0].effect
                }
              />
            );
          })}
      ...
      <PokePrincipalInfo>
        <InfoTitle>
          {" "}
          <FontAwesomeIcon icon={faFireFlameSimple} /> Moves
        </InfoTitle>
        <MoveList>
          {pokemonData.moves.map(({ move }, index) => {
            return <Move key={index}>{move.name}</Move>;
          })}
        </MoveList>
      </PokePrincipalInfo>
    </Main>
